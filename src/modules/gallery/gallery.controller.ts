import { accountRepository, eventRepository, galleryDriveConnectionRepository, galleryShareRepository, responseRepository, userRepository } from '@/modules/gallery/gallery.repository.js'
...
async function syncDriveForEvent(
  ...
  await galleryDriveConnectionRepository.upsert({
    where: {
      shareId_ownerEmail: {
        shareId: share.id,
  ...
  return galleryShareRepository.update({
...
export async function getGalleryShare(req: Request, res: Response) {
  try {
    ...
    const share = await galleryShareRepository.upsert({
      where: { eventId },
      update: {},
...
export async function updateGalleryShare(req: Request, res: Response) {
  try {
    ...
    const existing = await galleryShareRepository.findUnique({ where: { eventId } })
    const share = await galleryShareRepository.upsert({
      where: { eventId },
      update: {
...
export async function connectGalleryDrive(req: Request, res: Response) {
  try {
    ...
    const existingShare = await galleryShareRepository.upsert({
      where: { eventId },
      update: {},
      create: { eventId, token: createShareToken() },
      include: galleryShareInclude,
