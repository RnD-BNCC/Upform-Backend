import { pollRepository, pollSlideRepository, pollVoteRepository } from '@/modules/public-polls/public-polls.repository.js'
import type { Request, Response } from 'express'
import { addScore, getIO } from '@/config/socket.js'
import { aggregateResults } from '@/utils/poll-aggregation.js'
import { handleControllerError } from '@/utils/controller-error.js'
import type { PollCodeParams, CorrectArea } from '@/modules/public-polls/public-polls.types.js'
import type { PollSlideParams, PollVoteParams } from '@/modules/poll-slides/poll-slides.types.js'
...
export async function submitPollVote(req: Request<PollSlideParams>, res: Response) {
  try {
    ...
    if (slide.type === 'qa') {
      ...
      vote = await pollVoteRepository.create({
        data: { slideId, participantId: uniqueParticipantId, value, stsrc: 'A' },
      })
    } else {
      vote = await pollVoteRepository.upsert({
    ...
    if (isQuizSlide) {
      ...
      if (isCorrect) {
        let priorVotes = 0
        if (slide.type === 'multiple_choice' && correctAnswer) {
          priorVotes = await pollVoteRepository.count({
            where: {
              slideId,
              stsrc: { not: POLL_STSRC_DELETED },
          ...
          const prior = await pollVoteRepository.findMany({
            where: {
              slideId,
              stsrc: { not: POLL_STSRC_DELETED },
              createdAt: { lt: vote.createdAt },
...
export async function togglePollVoteAnswer(req: Request<PollVoteParams>, res: Response) {
  try {
    ...
    const vote = await pollVoteRepository.findFirst({
      where: {
        id: voteId,
        slideId,
        stsrc: { not: POLL_STSRC_DELETED },
    ...
    const updated = await pollVoteRepository.update({
      where: { id: voteId },
