import { emailBlastRepository, emailComposerDraftRepository, eventRepository, submitFormSettingRepository, unitOfWork } from '@/modules/email-blasts/email-blasts.repository.js'
...
} from '@/modules/email-blasts/email-blasts.types.js'

function normalizeSubmitFormSettingsBody(body: SubmitFormSettingsBody) {
  return {
    blocks: toPrismaJson(normalizeBlocks(body.blocks)),
...
export async function getEmailComposerDraft(
  ...
  try {
    ...
    const draft = await emailComposerDraftRepository.findUnique({
      where: { eventId },
    })
...
export async function saveEmailComposerDraft(
  ...
  try {
    ...
    const blocks = toPrismaJson(normalizeBlocks(body.blocks))

    const draft = await emailComposerDraftRepository.upsert({
      where: { eventId },
      create: {
        blocks,
        emailStyle,
...
export async function getSubmitFormSettings(
  ...
  try {
    ...
    const settings = await submitFormSettingRepository.findUnique({
      where: { eventId },
    })
...
export async function saveSubmitFormSettings(
  ...
  try {
    ...
    const settings = await submitFormSettingRepository.upsert({
      where: { eventId },
      create: {
        ...data,
        eventId,
...
export async function createEmailBlast(
  ...
  try {
    ...
    const blast = await unitOfWork.transaction(async (tx) => {
...
export async function listEmailBlasts(req: Request, res: Response) {
  try {
    ...
    const [blasts, total] = await Promise.all([
      ...
      emailBlastRepository.count({ where }),
    ])
...
export async function getEmailBlast(req: Request<EmailBlastParams>, res: Response) {
  try {
    const blast = await emailBlastRepository.findUnique({
...
export async function deleteEmailBlast(req: Request<EmailBlastParams>, res: Response) {
  try {
    const blast = await emailBlastRepository.findUnique({ where: { id: req.params.id } })
