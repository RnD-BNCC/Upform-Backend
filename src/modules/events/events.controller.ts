import { eventRepository, formAuditLogRepository, responseProgressRepository, responseRepository, sectionRepository, unitOfWork } from '@/modules/events/events.repository.js'
...
} from '@/modules/events/form-audit.service.js'
import {
  ...
  try {
    ...
    const [events, total, totalEvents, activeEvents, deletedEvents, totalResponses] =
      await Promise.all([
        ...
        eventRepository.count({ where: { stsrc: { not: EVENT_STSRC.deleted } } }),
        eventRepository.count({
          where: { status: 'active', stsrc: { not: EVENT_STSRC.deleted } },
        }),
        eventRepository.count({ where: { stsrc: EVENT_STSRC.deleted } }),
        responseRepository.count({ where: { stsrc: { not: EVENT_STSRC.deleted } } }),
      ])
...
export async function updateEvent(
  ...
  try {
    ...
    const event = await unitOfWork.transaction(async (tx) => {
...
export async function duplicateEvent(req: Request<EventParams>, res: Response) {
  try {
    const userEmail = getAuthEmail(res)
    const source = await eventRepository.findFirst({
      where: { id: req.params.id, stsrc: { not: EVENT_STSRC.deleted } },
      include: {
        sections: { orderBy: { order: 'asc' } },
      },
...
export async function saveBuilderEvent(
  ...
  try {
    ...
    const existingEvent = await eventRepository.findFirst({
    ...
    const existingSections =
      sectionIds.length > 0
        ? await sectionRepository.findMany({
...
export async function restoreEvent(req: Request<EventParams>, res: Response) {
  try {
    ...
    const [event] = await unitOfWork.transaction([
      ...
      responseProgressRepository.updateMany({
        where: existing.deletedAt
          ? { deletedAt: existing.deletedAt, eventId: existing.id }
...
export async function rollbackEventAuditLog(
  ...
  try {
    ...
    const log = await formAuditLogRepository.findFirst({
